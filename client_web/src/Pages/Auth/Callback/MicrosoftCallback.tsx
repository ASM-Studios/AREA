        ...
        const handleCallback = async () => {
            ...
            try {
                ...
                if (localStorage.getItem("jsonWebToken")) {
                    response = await instanceWithAuth.post(oauth.microsoft.bind, {
                        code,
                        code_verifier: codeVerifier,
                        redirect_uri: uri.microsoft.auth.redirectUri,
                    });
                } else {
                    response = await instance.post(oauth.microsoft.auth, {
                        code,
                        code_verifier: codeVerifier,
                        redirect_uri: uri.microsoft.auth.redirectUri,
                    })
                ...
                setError((error as Error)?.message || 'Failed to connect with Microsoft');
                setTimeout(() => {
                    navigate('/login');
                }, 2000);
            }
